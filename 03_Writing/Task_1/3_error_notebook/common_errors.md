---
layout: default
title: 寫作錯誤 - common_errors
---

# IELTS Task 1 Error Notebook 📝

## 🎯 Usage Guide

Use this guide to identify and eliminate recurring errors. Review before each practice session to keep these key points fresh.

---

## 📊 Error Classification Table

| Priority | Error Type                         | ❌ My Common Errors (Examples)                                                                                        | ✅ The Fix / Better Version                                                                                                                                                  | 🎯 Practice Method                                                              |
| -------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| **1**    | **Data Accuracy**<br>Data Accuracy | **Calculation Errors**<br>"The combination of these three groups' percentages... with combination percentage of 55%." | **Fix:** "The combination was 52% (16% + 20% + 16%)."<br><br>**Rule:** Your analysis is excellent, but final score depends on numerical accuracy.                            | Use calculator to verify all percentage and number calculations during practice |
| **2**    | **Grammar**<br>Grammar             | **Incorrect Passive Voice**<br>"users aging from 25 to 34 years old, was accounted for 20%..."                        | **Fix:** "...accounted for 20%..."<br><br>**Rule:** "Accounted for" is an active verb phrase. Subject (users) performs the action.                                           | Practice identifying active vs. passive voice usage                             |
| **3**    | **Vocabulary**<br>Vocabulary       | **Unnatural Phrases**<br>"users aging from 25 to 34..."<br>"...the ratio of this group came to third place."          | **Fix:** "users aged 25 to 34..."<br>"...this group ranked third" or "...was the third-largest group."<br><br>**Rule:** Pursue more standard academic expressions.           | Read academic articles to learn standard expressions                            |
| **4**    | **Grammar**<br>Grammar             | **Subject-Verb Agreement**<br>"users aging from... was accounted for..." (users = plural)                             | **Fix:** "users aging from... accounted for..." or "users... were..."<br><br>**Rule:** After identifying subject, ensure verb agrees with it (plural subject = plural verb). | Practice subject-verb agreement exercises, especially with plural subjects      |

---

## ✅ Quick Check List

Before submitting Task 1, please check:

- [ ] All numbers and percentages accurately calculated
- [ ] Verb voice correctly used
- [ ] Vocabulary natural and academic
- [ ] Subject-verb agreement correct
- [ ] Sentence structure clear

---

## 📚 Practice Suggestions

1. **Daily Practice:** Choose one error type to focus on
2. **Error Tracking:** Record errors from each practice session
3. **Regular Review:** Review this notebook weekly, update error patterns
4. **Example Learning:** Learn correct expressions from high-scoring sample essays

---

# My Personal IELTS Error Notebook

This log tracks recurring errors to eliminate from my writing. This combines feedback from both my coach and other AI tools like Grok.

_Last Updated: Aug 28, 2025_

| Priority | Error Type     | ❌ My Common Mistake (Example)        | ✅ The Fix / Better Version                                                               | 🧠 Mental Rule / Note                                                                                                                                                                                     |
| :------- | :------------- | :------------------------------------ | :---------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **1**    | **Grammar**    | **Sentence Fragments**                | `Which may result in...` <br> `Showing major influence...`                                | `..., which may result in...` <br> `..., showing major influence...`<br><br>**Rule:** `Which` and `-ing` clauses are "hooks," not "engines." They must be attached to the previous sentence with a comma. |
| **2**    | **Grammar**    | **Run-on / "Monster" Sentences**      | (Multiple examples of sentences over 40-50 words that combine too many ideas)             | Break the idea into 2-3 shorter, clearer sentences.                                                                                                                                                       | **Rule:** The **"25-Word Max Iron Law."** This is the #1 way to improve clarity and reduce errors.                       |
| **3**    | **Vocabulary** | **Keyword Spelling & Factual Errors** | `Cooper` -> `Copper` <br> `govenment` -> `government` <br> `Dorothy` the sheep -> `Dolly` | Proofread the final text against the prompt. Double-check famous names/facts.                                                                                                                             | **Rule:** Careless errors in keywords and famous examples undermine credibility. Add this to your 30-second final check. |
| **4**    | **Vocabulary** | **Imprecise Word Choice**             | `colleges` vs `colleagues` <br> `fell` vs `feel` <br> `loss` vs `lose`                    | Use the correct word based on meaning.                                                                                                                                                                    | **Rule:** These are personal "fossilized errors." Review this list before every practice to stay vigilant.               |
| **5**    | **Structure**  | **Incorrect Task 2 Structure**        | Writing a separate "Overall" paragraph after the introduction.                            | The thesis/outline is the **final part of the introduction**.                                                                                                                                             | **Rule:** Task 2 is a strict **four-paragraph** essay (Intro, Body 1, Body 2, Conclusion).                               |

---

## 🔧 Passive Voice Formula

**Passive Voice Form:** be verb + past participle (p.p.)

**Your Common Errors:**

- `...and being sent to...` → `...and **are sent** to...` (are + past participle sent)
- `...bricks will be shift to...` → `...bricks will be **shifted** to...` (be + past participle shifted)

**Action Note:** Write this formula in your "Error Notebook" and record these examples.

---

核心學習心得

心得一：任務完成度 (TA) - 你最鋒利的雙面刃

    學習成果：
    你證明了你擁有極其出色的高階分析能力。在「金屬價格圖」中，你準確理解了「變化百分比」這個最複雜的 Y 軸；在「老人活動圖」中，你的數據分析準確無誤，TA 拿到了 8.0 的高分。這證明你的天花板非常高。

    暴露出的問題：
    你的 TA 表現非常不穩定，容易被兩種「致命失誤」擊倒：

        根本性誤讀：在「人口增長圖」中，你將「年度增長數 (growth)」誤讀為「總人口 (total population)」，這是導致 5.5 分的主因。

        關鍵計算錯誤：在「遷移動機圖」中，你將 4.6 倍的差距算成了 7 倍，這個錯誤直接讓你失去了拿到 7.5 分的機會。

    行動建議：

        將「圖表身份證 (Chart ID Card)」內化於心：動筆前，必須用 10 秒鐘在心中確認：Y 軸的單位是什麼？（是數字、百分比、金額，還是增長率？）。這一步非做不可。

        將「數據計算」列為重點檢查對象：在你的「30 秒極速檢查法」中，必須優先檢查你自己計算過的數字（倍數、總和、差額）。

心得二：文法與結構 (GRA/CC) - 「短句策略」是你唯一的生命線

    學習成果：
    我們找到了能顯著提升你 GRA 和 CC 分數的「萬靈丹」——短句策略。在你嚴格遵守「每句話不超過 25 個單字」的練習中（如「鋼鐵供需圖」、「老人活動圖」），你的文章清晰度、文法準確率和邏輯連貫性都立刻提升到了 7.0-7.5 的水平。

    暴露出的問題：
    你還沒有將這個策略變成無意識的肌肉記憶。在最困難的「人口增長圖」練習中，你一緊張，就立刻放棄了短句，回到了「怪獸級長句」的老路，導致結構崩潰、錯誤百出，分數也應聲跌落。

    行動建議：

        將「短句策略」視為鐵律：這不再是一個選項，而是你在穩定達到 Band 8 之前必須遵守的紀律。它能保護你的文章，在你分析複雜圖表時，為你提供一個穩固的結構，防止崩盤。

心得三：詞彙精準度 (LR) - 從「知道」到「用對」

    學習成果：
    你已經成功消滅了一些最頑固的「老敵人」，例如錯誤的被動語態 was accounted for 和大部分的句子碎片。這證明你的「錯誤筆記本」是有效的。

    暴露出的問題：
    一些新的或潛在的慣性錯誤依然存在，需要在你的筆記本中「通緝」它們：

        關鍵字拼寫：Copper 拼錯。

        核心概念混淆：amount (用於不可數) vs. number (用於可數)。

        生硬片語：major/minor influence 這個片語的濫用。

        句子碎片：雖然大幅減少，但在「老人活動圖」中依然出現了。

    行動建議：

        主動更新「錯誤筆記本」：把以上這些問題立刻加進去。

        將「複習筆記」設為寫作第一步：每次動筆寫作前，必須先花一分鐘快速瀏覽你的筆記本，加深對這些錯誤的印象。

三、 最終總結與未來展望

經過這兩天的特訓，你的優勢和弱點都已清晰地展現在我們面前。你已經具備了所有衝擊高分的能力，現在需要的是將這些能力，在每一次下筆時都穩定地發揮出來。

你的下一個目標，不是學習任何新技巧，而是追求「穩定性 (Consistency)」。

每一次練習，都要像執行一份飛行前的檢查清單一樣，嚴格遵守我們總結出的規則：

    ✅ 檢查 Y 軸

    ✅ 遵守短句策略

    ✅ 複習錯誤筆記

    ✅ 執行 30 秒最終檢查（計算+拼寫）
    雅思 Task 1 錯誤筆記本

【使用說明】

    寫作前：花 2 分鐘快速瀏覽一遍，特別是你不熟悉的題型對應的要點。

    寫作中：時刻提醒自己遵守第二章的「短句鐵律」。

    寫完後：嚴格執行第一章和第三章提到的「30 秒極速檢查法」。

第一章：任務完成的「致命失誤」(Task Achievement "Fatal Flaws")

這些是會讓你的分數直接崩盤的根本性錯誤，必須最優先避免。
錯誤類型 ❌ 我的慣性錯誤 ✅ 正確/更好的版本 🧠 記憶規則
1.1 誤讀 Y 軸 「人口增長圖」中，將「年度增長數」理解為「總人口」。 準確描述為「人口增長數 (population growth)」。 圖表身份證 (Chart ID Card)：動筆前，心中默念 Y 軸的單位是數字、百分比、金額、還是增長率？
1.2 計算錯誤 「遷移動機圖」中，將 4.6 倍的差距計算為 7 倍。 176k ÷ 38k ≈ 4.6 times. 30 秒檢查：最終檢查時，必須重新心算一次你自己計算過的倍數、差額或總和。
1.3 主觀推測 「金屬價格圖」中，加入「...which shows its demand may be the largest.」的推測。 只描述事實：「...which was the highest growth rate of the three.」 黃金法則：只問「What (我看到了什麼)」，不問「Why (為什麼會這樣)」。只描述，不解釋！

第二章：文法結構的「頑固敵人」(Grammar & Structure "Stubborn Enemies")

這些是你反覆出現的結構性問題，直接影響文章的清晰度和準確性。
錯誤類型 ❌ 我的慣性錯誤 ✅ 正確/更好的版本 🧠 記憶規則
2.1 「怪獸級長句」 把 5-6 個資訊點塞進一個超過 50 字的句子裡，導致結構混亂。 每個句子只表達 1-2 個核心資訊點，用多個短句清晰呈現。 短句鐵律 (The Iron Law)：在穩定拿到 8.0 之前，任何句子都絕不超過 25 個單字。
2.2 句子碎片 Which became the highest... 或 Showing major influence... 獨立成句。 ..., which became the highest... 或 ..., showing major influence... 火車掛鉤理論：Which 和 -ing 動詞片語是車廂，必須用逗號掛在前面的火車頭（主句）上。
2.3 地圖題時態 描述 1986 年的地圖時用現在式；描述變化時沒用完成式。 描述過去 -> 過去式 (There were...)。<br>描述現在 -> 現在式 (There are...)。<br>描述變化 -> 現在完成式 (New houses have been built...)。 時空旅人規則：描述哪張圖，就用哪個時空的時態。描述兩張圖之間的「變化」，就用完成式來連接時空。
2.4 被動語態形式 ...will be shift to... 或 ...and being sent... ...will be shifted to... 或 ...and are sent... 被動語態公式：be 動詞 + 過去分詞 (p.p.)。檢查你的動詞是否是第三形態。

第三章：詞彙使用的「精準陷阱」(Vocabulary "Precision Traps")

這些是用詞不夠精準或不自然的問題，解決它們能讓你的文章質感大大提升。
錯誤類型 ❌ 我的慣性錯誤 ✅ 正確/更好的版本 🧠 記憶規則
3.1 關鍵字拼寫 Cooper Copper 30 秒檢查：最終檢查時，必須核對題目中給出的每一個關鍵名詞的拼寫。
3.2 核心概念混淆 the amount of meals (meals 可數)<br>the figure of pH (pH 是水平) the number of meals<br>the pH level Amount -> 不可數 (錢、水、時間)<br>Number -> 可數 (人、車、次數)<br>Level -> 用於刻度 (溫度、酸鹼度)<br>Figure -> 用於具體的獨立數字
3.3 生硬/不當片語 major/minor influence<br>less a third of<br>genres (用於描述食物) the most/least popular<br>less than a third of<br>types / kinds 多看少造：多學習高分範文的道地用法，而不是自己憑感覺創造片語。

---

## 📝 拼寫與詞形錯誤記錄

### 🔴 Critical 級別錯誤（最高優先級）

| 錯誤類型                | ❌ 錯誤範例   | ✅ 正確拼寫         | 🧠 記憶規則                            | 頻率   |
| ----------------------- | ------------- | ------------------- | -------------------------------------- | ------ |
| **Persistent Spelling** | colleges      | colleagues (同事)   | College 是教育場所；colleagues 是同事  | High   |
|                         | it's (所有格) | its (所有格)        | it's 永遠只表示 it is；所有格不加撇號  | High   |
|                         | cooperations  | corporations (企業) | Cooperation = 合作；corporation = 企業 | Medium |

### 🟡 High 級別錯誤（高優先級）

| 錯誤類型        | ❌ 錯誤範例     | ✅ 正確拼寫   | 頻率   |
| --------------- | --------------- | ------------- | ------ |
| **Spelling**    | extremelly      | extremely     | Medium |
|                 | populaution     | population    | Medium |
|                 | largestest      | largest       | Medium |
|                 | dessert         | desert (沙漠) | Low    |
|                 | govenment       | government    | Medium |
|                 | efficent        | efficient     | Medium |
|                 | material arts   | martial arts  | Low    |
|                 | inrational      | irrational    | Low    |
| **Word Choice** | soar (for pain) | sore (痠痛)   | Low    |
| **Word Form**   | can recovered   | can recover   | High   |
|                 | Following by    | Followed by   | Low    |

### 🟢 Medium 級別錯誤

| 錯誤類型      | ❌ 錯誤範例         | ✅ 正確拼寫             | 頻率   |
| ------------- | ------------------- | ----------------------- | ------ |
| **Spelling**  | atractive           | attractive              | Low    |
|               | fist                | first                   | Low    |
|               | woking              | working                 | Low    |
|               | accomodations       | accommodations          | Low    |
|               | disastrs            | disasters               | Low    |
|               | inconvinent         | inconvenient            | Low    |
|               | lasly               | lastly                  | Low    |
|               | tripple             | triple                  | Low    |
|               | through out         | throughout              | Low    |
|               | benificial          | beneficial              | Low    |
| **Word Form** | gaming addition     | gaming addiction (成癮) | Low    |
|               | loosing             | losing                  | Low    |
|               | loss motivations    | lose motivation         | Medium |
|               | fleshing            | fresh                   | Low    |
|               | new built           | newly built             | Medium |
|               | boxes               | socks                   | Low    |
|               | spins / weaved      | yarn / woven            | Low    |
|               | graduated form      | graduated from          | Low    |
|               | strategies thinking | strategic thinking      | Low    |
|               | Gooding             | Good                    | Low    |
|               | prisoning           | imprisonment (監禁)     | Low    |
|               | gun fires           | firearms (槍支)         | Low    |
|               | dis advantages      | disadvantages           | Low    |
|               | lossing             | losing                  | Medium |
|               | especial            | especially              | Low    |
|               | fiats               | fiat                    | Low    |
|               | complete removed    | completely removed      | Low    |
|               | edge-cutting        | cutting-edge   (最前沿的/領先的)         | Low    |

### 📊 錯誤頻率統計

- **Very High Frequency (🔥)**: colleges → colleagues, it's → its
- **High Frequency (⭐)**: can recovered → can recover
- **Medium Frequency**: spelling errors (government, efficient), word forms (losing, newly built)
- **Low Frequency**: minor spelling errors, uncommon word forms

### 🎯 記憶技巧

1. **常見拼寫錯誤記憶口訣：**

   - Government = Govern + ment (治理者)
   - Efficient = prefix ef- + ficient (效力)
   - Parliament = 議會，記住中間有"lia"

2. **詞形變化記憶點：**

   - 情態動詞後跟原形動詞：can/should/will + 動詞原形
   - 形容詞變副詞：new → newly; complete → completely
   - 動詞變名詞：lose → loss (動詞變名詞)

3. **檢查清單：**
   - [ ] 檢查常見拼寫錯誤（government, efficient）
   - [ ] 確認詞性運用正確（lose vs loss）
   - [ ] 驗證情態動詞後詞形（can + 動詞原形）
   - [ ] 檢查副詞形式（newly, completely）

---
